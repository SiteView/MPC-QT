...
void InformationAnalyze::vCompare()
{
    ...
    for (int i = 0; i < serverSoftwareList.size(); ++i)
    {
        ...
        else
        {
            ...
            qDebug() << "l_sVersionServer" << l_sVersionServer;
            qDebug() << "l_sVersionLocal" << l_sVersionLocal;
            ...
            QString l_sUninstallString;
            l_sUninstallString = l_setLocal->value(l_sSoftwareName + "/uninstallstring").toString();
            l_psetUpdate->setValue(l_sSoftwareNameGBK + "/uninstallstring", l_sUninstallString);
...
void InformationAnalyze::vInitLocalInfomation()
{
    ...
    foreach(QString group,groupsList)
    {
        ...
        QString l_sSoftwareName=l_setReg.value("DisplayName").toString().trimmed();
        //QString l_sSoftwareVersion = l_setReg.value("DisplayVersion").toString();
        QString l_sSoftwareVersion = l_setReg.value("DisplayVersion").toString().trimmed();


        //aaron 0408uninstall
        QString l_sUninstallString = l_setReg.value("UninstallString").toString();
        ...
        if(!l_sSoftwareName.isEmpty())
        {
            ...
            //aaron 0408 uninstall
            l_psetLocal->setValue(l_sSoftwareName + "/uninstallstring", l_sUninstallString);
        }
        l_setReg.endGroup();
    }
