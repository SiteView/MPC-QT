...
void HttpDownload::connectStateChanged(int state)
{
    ...
    case QHttp::Unconnected:
        ...
        if (retryString != "")
        {
            ...
            downloadThreadWaitTimer = new QTimer(this);
            connect(downloadThreadWaitTimer, SIGNAL(timeout()), downloadThread, SLOT(setThreadStopped()));
            downloadThreadWaitTimer->start(retryDelayInt*1000);
            retryString = "";
        }
        else
        {
