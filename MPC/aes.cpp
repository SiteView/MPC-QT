...
AES::AES(QString key)
{
    QByteArray l_baTemp = key.toLatin1();
    char * l_pInBuffer = l_baTemp.data();
    AES((unsigned char*)l_pInBuffer);
}
 ...
 * @返回值： void
 *********************************************/
void AES::OnAesEncrypt(const QString &sInBuffer, QString &sOutEncryptData)
{
    ...
    Bm53Cipher(l_pInBuffer,  l_pOutEncrytData);
...
void AES::OnAesUncrypt(const QString &sInBuffer, QString &sOutUncryptData)
{
    QByteArray l_baTemp = sInBuffer.toLatin1();
    //aaron 0327
    char * l_pInBuffer = qstrdup(l_baTemp.data());
    char l_pOutUncrytData[4086];

    Bm53InvCipher(l_pInBuffer, l_pOutUncrytData);


    sOutUncryptData = QString(l_pOutUncrytData);
