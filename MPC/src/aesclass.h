...
public:
	AesClass();
    AesClass(int keySize, const QString& sKeyBytes);
	~AesClass();
  void InitializePrivateKey(DWORD KeySize,UCHAR *KeyBytes);                      //AES 密钥初始化
  void InitializePrivateKey(DWORD KeySize, QString sKeyBytes);
  DWORD OnAesEncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer);			//AES 加密数据
    ...
    DWORD OnAesEncrypt(const QString& sInBuffer, QString& sOutEncryptData);
    DWORD OnAesUncrypt(const QString& sInBuffer, QString& sOutUncryptData);
    //AES 解密数据
private:
	Aes  * m_lpAes;
};
