...
AesClass::AesClass(int keySize, const QString &sKeyBytes)
{
    m_lpAes = NULL;
    InitializePrivateKey(keySize, sKeyBytes);
}
AesClass::~AesClass()
{
	if (m_lpAes!=NULL)
...
void AesClass::InitializePrivateKey(DWORD KeySize, QString sKeyBytes)
{

    QByteArray l_baTemp = sKeyBytes.toLatin1();
    UCHAR *p = (UCHAR *)(l_baTemp.data());
    InitializePrivateKey(KeySize, p);
}

VOID AesClass::InitializePrivateKey(DWORD KeySize,UCHAR *KeyBytes)
{
	if (m_lpAes)
	{
		delete m_lpAes;
...
DWORD AesClass::OnAesEncrypt(const QString &sInBuffer, QString &sOutEncryptData)
{
    QByteArray l_baTemp = sInBuffer.toLatin1();
    char * l_pInBuffer = l_baTemp.data();

    DWORD size = strlen(l_pInBuffer);

    char l_pOutEncrytData[1024];
    DWORD dReturn = OnAesEncrypt((LPVOID)l_pInBuffer, size, (LPVOID)l_pOutEncrytData);

    sOutEncryptData = l_pOutEncrytData;
...
DWORD AesClass::OnAesEncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
	DWORD OutLength=0;
...
DWORD AesClass::OnAesUncrypt(const QString &sInBuffer, QString &sOutUncryptData)
{
    QByteArray l_baTemp = sInBuffer.toLatin1();
    char * l_pInBuffer = l_baTemp.data();

    DWORD size = sizeof(l_pInBuffer);

    char l_pOutUncrytData[1024];

    DWORD dReturn = OnAesUncrypt((LPVOID)l_pInBuffer, 1024,(LPVOID)l_pOutUncrytData);

    sOutUncryptData = l_pOutUncrytData;
...
DWORD AesClass::OnAesUncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
	DWORD OutLength=0;
	if (m_lpAes==NULL||OutBuffer==NULL)
	{
